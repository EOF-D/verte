...
\vertec\ is a compiler built for the \verte\ programming language which currently utilizes \llvm\ for machine code generation.
With the proposed project, the \llvm\ backend will be replaced with a custom, hand-written \asm\ backend. 
This new backend will directly translate the \code{AST} (abstract syntax tree) created by the current frontend into \asm\ instructions.
